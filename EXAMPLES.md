...
# ⚠️  Порт 8005 свободен (запустите бэкенд)
# ⚠️  Порт 3005 свободен (запустите фронтенд)
...
# Выводит:
# ═══════════════════════════════════════
# Запуск Django на 0.0.0.0:8005...
# Ваш IP в локальной сети: 192.168.1.240
# Подключайтесь по адресу: http://192.168.1.240:8005
# ═══════════════════════════════════════
...
# Ваш IP в локальной сети: 192.168.1.240
# Подключайтесь по адресу: http://192.168.1.240:3005
# ═══════════════════════════════════════

# На телефоне:
# 1. Подключиться к той же WiFi сети
# 2. Открыть браузер
# 3. Ввести: http://192.168.1.240:3005
...
#    ⚠️  Порт 8005 свободен (запустите бэкенд)  ← ПРОБЛЕМА!
#    ✅ Порт 3005 (Vite) занят
# 
...
./run_backend.sh
# Запускает: http://192.168.1.240:8005
# Говорит команде: "Бэк на 192.168.1.240:8005"
...
# Браузер: http://localhost:3005
# Показать работу приложения

# Демонстрация 2: Доступ по IP
# Взять телефон, открыть: http://192.168.1.240:3005
...
# Открыть DevTools → Network → показать запросы к 192.168.1.240:8005
...
curl http://localhost:8005/api/drugs/ | jq

# Тест 2: Запрос по IP
curl http://192.168.1.240:8005/api/drugs/ | jq

# Тест 3: С другого устройства (телефон/планшет)
# Установить termux или использовать браузер DevTools:
fetch('http://192.168.1.240:8005/api/drugs/')
  .then(r => r.json())
  .then(console.log)
```
...
netstat -tuln | grep 8005      # Проверить бэкенд
netstat -tuln | grep 3005      # Проверить фронтенд
