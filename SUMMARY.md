...
- **`frontend/vite.config.ts`**:
  ...
  - Прокси настроен на `0.0.0.0:8000`
...
#### `./run_backend.sh`
Запуск Django на `0.0.0.0:8000` с отображением IP

#### `./run_frontend.sh`
Запуск React на `0.0.0.0:3000` с отображением IP
...
┌─────────────────┐
│   Браузер       │
│  localhost:3000 │ или 192.168.1.XXX:3000
└────────┬────────┘
         │
         │ fetch('/api/...')
         ↓
...
curl http://192.168.1.XXX:8000/api/drugs/
```
...
4. **Firewall**: Убедитесь, что порты 3000 и 8000 не заблокированы
...
- [ ] Firewall не блокирует порты 3000, 8000
